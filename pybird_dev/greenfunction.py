...
class GreenFunction(object):
    def __init__(self, Omega0_m, w=None, quintessence=False):
        ...
        self.OmegaL_by_Omega_m = (1.0 - self.Omega0_m) / self.Omega0_m
        self.wcdm = False
    ...
    def C(self, a):
        if self.quintessence:
            return 1.0 + (1.0 + self.w) * self.OmegaL_by_Omega_m * a ** (-3.0 * self.w)
        else:
            return 1.0

    def H(self, a):
        """Conformal Hubble"""
        if self.wcdm or self.quintessence:
            return (self.Omega0_m / a + (1.0 - self.Omega0_m) * a ** 2 * a ** (-3.0 * (1.0 + self.w))) ** 0.5
        else:
            return (self.Omega0_m / a + (1.0 - self.Omega0_m) * a ** 2) ** 0.5
    ...
    def D(self, a):
        ...
        if self.wcdm:
            return a * hyp2f1(
                ...
                -1 / (3 * self.w),
                1 - (5 / (6 * self.w)),
                -(a ** (-3 * self.w)) * self.OmegaL_by_Omega_m,
            )
        else:
            I = quad(self.H3, 0, a)[0]
            return 5 * self.Omega0_m * I * self.H(a) / (2.0 * a)

    def DD(self, a):
        """Derivative of growth factor"""
        if self.wcdm:
            return -(a ** (-3.0 * self.w)) * self.OmegaL_by_Omega_m * (
                (3 * (self.w - 1)) / (6.0 * self.w - 5.0)
            ) * hyp2f1(
                1.5 - 0.5 * (1 / self.w),
                1 - (1 / (3.0 * self.w)),
                2 - (5 / (6.0 * self.w)),
                -(a ** (-3.0 * self.w)) * self.OmegaL_by_Omega_m,
            ) + hyp2f1(
                (self.w - 1) / (2.0 * self.w),
                -1 / (3.0 * self.w),
                1 - (5 / (6.0 * self.w)),
            ...
            return (2.5 - (1.5 * self.D(a) / a)) * self.Omega_m(a) * self.C(a)
    ...
    def Dminus(self, a):
        ...
        if self.wcdm:
            ...
            return self.H(a) / (a * self.Omega0_m ** 0.5)
    ...
    def G2t(self, a, ai):
        return (
            a
            * self.fplus(ai)
            * (self.DDminus(a) * self.D(ai) - self.DD(a) * self.Dminus(ai))
            / (self.fplus(a) * ai * ai * self.W(ai))
        )
    ...
    def Y(self, a):
        return -3 / 14.0 + self.mV11d(a) + self.mV12d(a)
