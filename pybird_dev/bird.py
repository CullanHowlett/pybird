...
class Bird(object):
    ...
    def __init__(
        self, cosmology=None, with_bias=True, with_stoch=False, with_nlo_bias=False, with_assembly_bias=False, co=co
    ):
        self.co = co
        ...
        if self.with_stoch:
            if self.co.with_cf:
                ...
                self.Cstl[0, 0] = (
                    np.exp(-self.co.km * self.co.s) * self.co.km ** 2 / (4.0 * np.pi * self.co.s) / self.co.nd
                )
                self.Cstl[0, 1] = (
                    -self.co.km ** 2 * np.exp(-self.co.km * self.co.s) / (4.0 * np.pi * self.co.s ** 2) / self.co.nd
                )
                self.Cstl[0, 2] = (
                    np.exp(-self.co.km * self.co.s)
                    * (3.0 + 3.0 * self.co.km * self.co.s + self.co.km ** 2 * self.co.s ** 2)
                    / (4.0 * np.pi * self.co.s ** 3)
                    / self.co.nd
                )
            else:
                self.bst = np.zeros(shape=(self.co.Nst))
    ...
    def setBias(self, bias):
        ...
        if self.with_nlo_bias:
            self.bnlo[0] = 2.0 * b1 ** 2 * bias["bnlo"] / self.co.km ** 4

        if self.with_assembly_bias:
            bq = bias["bq"]
        ...
        if self.with_bias:
            for i in range(self.co.Nl):
                ...
                if self.with_assembly_bias:
                    (b1 - bq / 3.0) ** 2 * mu[0][l] + 2.0 * (b1 - bq / 3.0) * (f + bq) * mu[2][l] + (f + bq) ** 2 * mu[
                        4
                    ][l]
                else:
            ...
            self.bct = np.array([2.0 * b1 * b5, 2.0 * b1 * b6, 2.0 * b1 * b7, 2.0 * f * b5, 2.0 * f * b6, 2.0 * f * b7])
    ...
    def setPs(self, bs, setfull=True):
        ...
        if self.with_nlo_bias:
            self.Ps[1] += np.einsum("l,x,x->lx", self.bnlo, self.co.k ** 4, self.P11)
        if setfull:
            self.setfullPs()
    ...
    def setCf(self, bs, setfull=True):
        ...
        if self.with_nlo_bias:
            self.Cf[1] += np.einsum("l,lx->lx", self.bnlo, self.Cnlo)
        if setfull:
            self.setfullCf()
    ...
    def setreducePslb(self, bs):
        ...
        if self.with_stoch:
            Ps1 += np.einsum("b,lbx->lx", self.bst, self.Pstl)
        if self.with_nlo_bias:
            Ps1 += np.einsum("l,lbx->lx", self.bnlo, self.Pnlol)
    ...
    def setreduceCflb(self, bs):
        ...
        if self.with_stoch:
            Cf1 += np.einsum("b,lbx->lx", self.bst, self.Cstl)
        if self.with_nlo_bias:
            Cf1 += np.einsum("l,lbx->lx", self.bnlo, self.Cnlol)
    ...
    def formatTaylorPs(self, kdata=None, Ps=None):
        ...
        if Ps is None:
            P11l, Ploopl, Pctl, Pnlol = self.P11l, self.Ploopl, self.Pctl, self.Pnlol
        else:
            P11l, Ploopl, Pctl, Pnlol = Ps
        ...
        if self.with_nlo_bias:
            Ploop3 = np.concatenate(np.einsum("lnk->lkn", Pnlol), axis=0)
            Ploop = np.hstack((allk, Ploop1, Ploop2, Ploop3))
        else:
            Ploop = np.hstack((allk, Ploop1, Ploop2))
        return Plin, Ploop

    def formatTaylorCf(self, sdata=None, CFs=None):
        """ An auxiliary to pipe PyBird with TBird: puts Bird(object) power spectrum multipole terms into the right shape for TBird """
        ...
        if CFs is None:
            C11l, Cloopl, Cctl, Cnlol = self.C11l, self.Cloopl, self.Cctl, self.Cnlol
        else:
            C11l, Cloopl, Cctl, Cnlol = CFs
        ...
        Ploop2 = np.concatenate(np.einsum("lnk->lkn", Ploop2), axis=0)
        if self.with_nlo_bias:
            Ploop3 = np.concatenate(np.einsum("lnk->lkn", Cnlol), axis=0)
            Ploop = np.hstack((allk, Ploop1, Ploop2, Ploop3))
        else:
            Ploop = np.hstack((allk, Ploop1, Ploop2))
        return Plin, Ploop
    ...
    def setw(self, bs):
        ...
        self.setBias(bs)
        self.w = (
            np.einsum("n,nx->x", self.b11, self.wlin)
            + np.einsum("n,nx->x", self.bct, self.wct)
            + np.einsum("n,nx->x", self.bloop, self.wloop)
        )
        if self.with_nlo_bias:
            self.w += np.einsum("n,nx->x", self.bnlo, self.wnlo)
