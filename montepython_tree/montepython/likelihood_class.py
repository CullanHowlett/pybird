    ...
    def __init__(self, path, data, command_line):
        ...
        try:
            ...
            (
                ...
                self.__load_gaussian_data()
            )  # with gaussian case: column 1: k[h/Mpc]  column 2-N+2: signal  column N+3-2N+2: error
        ...
        if self.Nw is not 0:
            ...
            dkmax = (self.kmax1 - self.kmax0) / (self.Nw - 1.0)
            kmask0 = np.argwhere((self.k <= self.kmax0) & (self.k >= self.kmin))[:, 0]
            self.kmask = kmask0
            for i in range(self.Nw - 1):
                kmaski = (
                    np.argwhere((self.k <= self.kmax0 + (i + 1) * dkmax) & (self.k >= self.kmin))[:, 0]
                    + (i + 1) * self.Nk
                )
            ...
            try:
                if self.use_prior and self.priors is not None:
                    ...
                    if self.model == 1:
                        ...
                        print(
                            "EFT priors: b3: %s, cct: %s, cr1(+cr2): %s, ce2: %s, shotnoise: %s"
                            % (b3, cct, cr1, ce2, sn)
                        )
                    elif self.model == 2:
                ...
                if self.model == 1:
                    ...
                    print(
                        "EFT priors: b3: %s, cct: %s, cr1(+cr2): %s, ce2: %s, ce1: %s (default)"
                        % (b3, cct, cr1, ce2, ce1)
                    )
                elif self.model == 4:
            ...
            try:
                if self.use_prior and self.priors is not None:
                    ...
                    if self.model == 1:
                        ...
                        print(
                            "EFT priors: b3: %s, cct: %s, cr1: %s, cr2: %s, ce2: %s, shotnoise: %s"
                    ...
                    self.use_prior = True
                    if self.model == 1:
                        self.priors = np.array([10.0, 10.0, 10.0, 10.0, 10.0, 2.0])
                    elif self.model == 2:
                        self.priors = np.array([10.0, 10.0, 10.0, 10.0, 10.0])
                    elif self.model == 3:
                        self.priors = np.array([10.0, 10.0, 10.0, 10.0, 10.0, 10.0])
            except:
                self.use_prior = True
                if self.model == 1:
...
class Likelihood_bird(Likelihood_eft):
    def __init__(self, path, data, command_line):
        ...
        self.need_cosmo_arguments(data, {"output": "mPk", "z_max_pk": self.z, "P_k_max_h/Mpc": 1.0})
    ...
    def loglkl(self, cosmo, data):
        ...
        bs = [b1, b2, bval[2], b4, bval[4] / self.knl ** 2, bval[5] / self.km ** 2, 0.0]
        ...
        if self.birdlkl is "fastmarg" or self.birdlkl is "fastfull":
            if data.need_cosmo_update is True or self.bird is None:
                ...
                DA = cosmo.angular_distance(self.z) * cosmo.Hubble(0.0)
                H = cosmo.Hubble(self.z) / cosmo.Hubble(0.0)
            ...
            if self.cf:
                self.bird.setreduceCflb(bs)
            else:
                self.bird.setreducePslb(bs)

            if self.birdlkl is "fastmarg":
                if self.cf:
                    self.bird.Cb3 = self.bird.Cloopl[:, 3] + b1 * self.bird.Cloopl[:, 7]
                else:
        ...
        if self.cf:
            modelX = self.bird.fullCf.reshape(-1)
        else:
            modelX = self.bird.fullPs.reshape(-1)

        if self.with_bao:  # BAO
            DM_at_z = cosmo.angular_distance(self.zbao) * (1.0 + self.zbao)
        ...
        if "marg" in self.birdlkl:
            if self.cf:
                Pi = self.__get_Pi_for_marg(self.bird.Cctl, self.bird.Cb3, b1, self.bird.f, model=self.model)
            else:
        ...
        if self.use_prior:
            ...
            if self.model <= 4:
                prior += 0.5 * (bval[9] / self.nd / self.km ** 2 / self.priors[3]) ** 2  # ce,l2
            if self.model == 1:
                prior += -0.5 * (bval[7] / self.nd / self.priors[4]) ** 2  # ce0
            if self.model == 3:
                prior += -0.5 * (bval[8] / self.nd / self.km ** 2 / self.priors[4]) ** 2  # ce,l0
    ...
    def __get_Pi_for_marg(self, Pct, Pb3, b1, f, model=2):
        ...
        if model <= 4:
            ...
            if self.use_wedge:
                kp2l2 = self.Projection.Wedges_external(kp2l2)
...
class Likelihood_taylor(Likelihood_eft):
    def __init__(self, path, data, command_line):
        ...
        self.linder = np.load(
            os.path.abspath(os.path.join(self.data_directory, self.gridpath, "DerPlin%s.npy" % self.gridname)),
            allow_pickle=True,
        )
        self.loopder = np.load(
            os.path.abspath(os.path.join(self.data_directory, self.gridpath, "DerPloop%s.npy" % self.gridname)),
            allow_pickle=True,
        )
    ...
    def loglkl(self, cosmo, data, marg_gaussian=True):
        ...
        try:
            ...
            dtheta = np.array(
                (
                    data.cosmo_arguments["A_s"] - self.central[0],
        ...
        if marg_gaussian:
            ...
            chi2 = chi2mar + chi2nomar - 5 * np.log(2.0 * np.pi)
        else:
            chi2 = np.dot(modelX - self.ydata, np.dot(self.invcov, modelX - self.ydata))
    ...
    def __get_Pi_for_marg(self, Ploop, kfull, b1, model=2, withBisp=False):
        ...
        if model == 1:
            ...
            Pi = np.array(
                [
                    2.0 * Ploop[:, 3, :] + b1 * Ploop[:, 7, :],
                    2.0 * (Ploop[:, 15, :] + b1 * Ploop[:, 12, :]) / self.knl ** 2,
                    4.0 * (Ploop[:, 16, :] + b1 * Ploop[:, 13, :]) / self.km ** 2,
                    4.0 * (Ploop[:, 17, :] + b1 * Ploop[:, 14, :]) / self.km ** 2,  # b7
                    2400.0 * Onel0,
                    2.0 * kl2 ** 2 / self.nd / self.km ** 2,
                ]
            )  # this is k2 quad
        elif model == 2:
            kl2 = np.array([np.zeros(nk), kfull])
            Pi = np.array(
                [
                    2.0 * (Ploop[:, 3, :] + b1 * Ploop[:, 7, :]),  # b3
                    2.0 * (Ploop[:, 15, :] + b1 * Ploop[:, 12, :]) / self.knl ** 2,  # b5
                    4.0 * (Ploop[:, 16, :] + b1 * Ploop[:, 13, :]) / self.km ** 2,  # b6
                    4.0 * (Ploop[:, 17, :] + b1 * Ploop[:, 14, :]) / self.km ** 2,  # b7
                    2.0 * kl2 ** 2 / self.nd / self.km ** 2,
                ]
            )
