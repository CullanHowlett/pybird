...
def get_pder_lin(pi, dx, filename):
    ...
    dpdx = np.array(
        [findiff.FinDiff((i, dx[i], 1), acc=4)(pi)[idx, idx, idx, idx, idx, :, :, :] for i in range(lenpar)]
    )
    t0 = time.time()
    ...
    # Second derivatives
    d2pdx2 = np.array(
        [findiff.FinDiff((i, dx[i], 2), acc=2)(pi)[idx, idx, idx, idx, idx, :, :, :] for i in range(lenpar)]
    )
    t1 = time.time()
    ...
    d2pdxdy = np.array(
        [
            [i, j, findiff.FinDiff((i, dx[i], 1), (j, dx[j], 1), acc=2)(pi)[idx, idx, idx, idx, idx, :, :, :]]
    ...
    d3pdx2dy = np.array(
        [
            [i, j, findiff.FinDiff((i, dx[i], 2), (j, dx[j], 1))(pi)[idx, idx, idx, idx, idx, :, :, :]]
            for (i, j) in combinations(range(lenpar), 2)
        ]
    )
    t0 = time.time()
    ...
    d3pdxdy2 = np.array(
        [
            [i, j, findiff.FinDiff((i, dx[i], 1), (j, dx[j], 2))(pi)[idx, idx, idx, idx, idx, :, :, :]]
            for (i, j) in combinations(range(lenpar), 2)
        ]
    )
    t1 = time.time()
    ...
    d3pdxdydz = np.array(
        [
            [
                ...
                j,
                k,
                findiff.FinDiff((i, dx[i], 1), (j, dx[j], 1), (k, dx[k], 1))(pi)[idx, idx, idx, idx, idx, :, :, :],
            ]
...
def get_pder_loop1(pi, dx, filename):
    ...
    dpdx = np.array(
        [findiff.FinDiff((i, dx[i], 1), acc=4)(pi)[idx, idx, idx, idx, idx, idx, :, :, :] for i in range(lenpar)]
    )
    t0 = time.time()
    print("Done dpdx in %s sec" % str(t1 - t0))
    ...
    # Second derivatives
    d2pdx2 = np.array(
        [findiff.FinDiff((i, dx[i], 2), acc=2)(pi)[idx, idx, idx, idx, idx, idx, :, :, :] for i in range(lenpar)]
    )
    t1 = time.time()
    print("Done d2pdx2 in %s sec" % str(t1 - t0))
    ...
    d2pdxdy = np.array(
        [
            [i, j, findiff.FinDiff((i, dx[i], 1), (j, dx[j], 1), acc=2)(pi)[idx, idx, idx, idx, idx, idx, :, :, :]]
            for (i, j) in combinations(range(lenpar), 2)
        ]
    )
    t0 = time.time()
    ...
    # Third derivatives: we only need it for A_s, so I do this by hand
    d3pdx3 = np.array(
        [findiff.FinDiff((i, dx[i], 3))(pi)[idx, idx, idx, idx, idx, idx, :, :, :] for i in range(lenpar)]
    )
    t1 = time.time()
    print("Done d3pdx3 in %s sec" % str(t1 - t0))
    allder = (p0, dpdx, d2pdx2, d2pdxdy, d3pdx3)
...
def get_pder_loop2a(pi, dx, filename):
    ...
    d3pdx2dy = np.array(
        [
            [i, j, findiff.FinDiff((i, dx[i], 2), (j, dx[j], 1))(pi)[idx, idx, idx, idx, idx, idx, :, :, :]]
            for (i, j) in combinations(range(lenpar), 2)
        ]
    )
    t0 = time.time()
...
def get_pder_loop2b(pi, dx, filename):
    ...
    d3pdxdy2 = np.array(
        [
            [i, j, findiff.FinDiff((i, dx[i], 1), (j, dx[j], 2))(pi)[idx, idx, idx, idx, idx, idx, :, :, :]]
            for (i, j) in combinations(range(lenpar), 2)
        ]
    )
    t1 = time.time()
    ...
    d3pdxdydz = np.array(
        [
            [
                ...
                j,
                k,
                findiff.FinDiff((i, dx[i], 1), (j, dx[j], 1), (k, dx[k], 1))(pi)[idx, idx, idx, idx, idx, idx, :, :, :],
            ]
            for (i, j, k) in combinations(range(lenpar), 3)
        ]
    )
...
def get_bias(d_fit):
    ...
    vloop = np.array(
        [
            ...
            d_fit["b2"] * d_fit["b4"],
            d_fit["b4"] ** 2,
            d_fit["b1"] * d_fit["b5"] / knl ** 2,
            d_fit["b1"] * d_fit["b6"] / km ** 2,
            d_fit["b1"] * d_fit["b7"] / km ** 2,
            d_fit["b5"] / knl ** 2,
            d_fit["b6"] / km ** 2,
            d_fit["b7"] / km ** 2,
        ]
