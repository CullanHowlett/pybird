...
class FFTLog(object):
    ...
    def setPow(self):
        self.Pow = self.bias + 1j * 2.0 * np.pi / (self.Nmax * self.dx) * (np.arange(self.Nmax + 1) - self.Nmax / 2.0)
    ...
    def Coef(self, xin, f, window=1, co=common):
        ...
        for i in range(self.Nmax):
            if xin[0] > self.x[i]:
                fx[i] = Aslow * self.x[i] ** nslow * np.exp(-self.bias * i * self.dx)
            elif xin[-1] < self.x[i]:
                fx[i] = Ashigh * self.x[i] ** nshigh * np.exp(-self.bias * i * self.dx)
            else:
                fx[i] = interpfunc(self.x[i]) * np.exp(-self.bias * i * self.dx)
