...
truecrd = [
    valueref[l] + delta[l] * np.arange(-order, order + 1) for l in range(len(freepar))
]  # list -i_n, ..., +i_n where to evaluate each freepar
...
def get_masses(sum_masses, hierarchy="NH"):
    ...
    if "n" in hierarchy.lower():
        ...
        def m1_func(m1, M_tot):
            return (
                M_tot ** 2 - (m1 + np.sqrt(m1 ** 2 + delta_m_squared_21) + np.sqrt(m1 ** 2 + delta_m_squared_31)) ** 2
            )

        m1, opt_output, success, output_message = fsolve(
            m1_func, sum_masses / 3.0, (sum_masses), full_output=True, xtol=1e-04, maxfev=500
        )
        m1 = m1[0]
        m2 = (delta_m_squared_21 + m1 ** 2.0) ** 0.5
        m3 = (delta_m_squared_31 + m1 ** 2.0) ** 0.5
        return m1, m2, m3
    else:
        return None
...
def CompPterms(pardict, kmin=0.001, kmax=0.31):
    ...
    this_plin = plinear.LinearPower(parlinear, np.logspace(np.log10(1.0e-7), np.log10(2.01), 150))
    this_plin.compute()
