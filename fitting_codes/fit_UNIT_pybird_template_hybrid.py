...
def do_emcee(func, start):
    ...
    fmt_str = (
        "%s_%s_%2dhex%2d_%s_%s_%s_hybrid.hdf5" if pardict["do_corr"] else "%s_%s_%3.2lfhex%3.2lf_%s_%s_%s_hybrid.hdf5"
    )
...
def lnprior(params, birdmodel):
    ...
    if np.any(np.less([birdmodel.valueref[0], birdmodel.valueref[1], omega_cdm, omega_b], lower_bounds)) or np.any(
        np.greater([birdmodel.valueref[0], birdmodel.valueref[1], omega_cdm, omega_b], upper_bounds)
    ):
        return -np.inf
...
def lnlike(params, birdmodel, fittingdata, plt):
    ...
    # Get the bird model
    Plin, Ploop = birdmodel.compute_hybrid(params[:4])
...
if __name__ == "__main__":
    ...
    birdmodel = BirdModel(pardict, template=True, direct=False)
