...
if __name__ == "__main__":
    ...
    if do_corr:
        chainfile = (
            "/Volumes/Work/UQ/DESI/MockChallenge/Pre_recon_HandShake/chain_xi_UNIT_HODsnap97_ELGv1_s40-200_3order.dat"
        )
    else:
    ...
    start = np.array(
        [
            valueref[0],
            valueref[1],
            (valueref[2] + valueref[3]) / valueref[1] ** 2,
            1.3,
            1.0,
            1.0,
            1.0,
    ...
    if do_marg:
        begin = [
            [
                (dx[0] * (np.random.rand() - 0.5) + 1.0) * result["x"][0],
                (dx[1] * (np.random.rand() - 0.5) + 1.0) * result["x"][1],
                (dx[2] * (np.random.rand() - 0.5) + 1.0) * result["x"][2],
